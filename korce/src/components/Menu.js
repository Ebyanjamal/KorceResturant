...
const Menu = () => {
  ...
  <div className='bg-yellow-200 h-[780px] bg-menu absolute w-full
 ...
 <img src={modelImg} alt= " "/>
   ...
   className='relative top-80 lg"top-96'>
    <div className='.container mx auto'>
      <div className='lg-mt-24 m-h-[590px] z-30 mb-8 md:mb-4 xl:mb-16'>
        <div className='grid grid-cols-1 lg:grid-cols-4 min-h-[590px]'>
          {menuItems.map((item,index) => {
            const{ image,name,price,description} =item
            return <div key={index}>
              <div className='flex flex-row lg:flex-col h-full'>
                <div className={`w-[45%] md:w-auto ${index === 1 || index === 3 ? 'lg:order-1' : ''}`}> 
                  <img src={image} alt=''/>
                </div>
                <div className='bg-[#fff3e4] flex-1 flex flex-col justify-center
                px-6 lg:p-12 lg:max-h[250px] xl:max-h-max'>
                  <div className='text-center' >
                    <div className='text-xl font-semibold text-dark xl:text-2xl'>{name}</div>
                    <div className='my-1 text-[20px] lg:text-[30px] lg:my-6 text-accent font-semibold'>{price}</div>
                    <div>{description}</div>
                  </div>
                </div>
              </div>
